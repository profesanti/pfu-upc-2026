# FPU UPC 2026 (Español)

Ayudas de Formación de Profesorado Universitario para la realización de una tesis doctoral en la UPC

**Texto fuente:** [`fpu-upc-2026/fpu-upc-2026-webpage-text-en.md`](./fpu-upc-2026/fpu-upc-2026-webpage-text-en.md)
**URL original:** https://talenthub.upc.edu/en/jobs/r1/fpu-upc-2026/fpu-upc-2026
**Imagen principal (local):** [`fpu-upc-2026/image-800-9046d66ac1126713224946444e2d996c.png`](./fpu-upc-2026/image-800-9046d66ac1126713224946444e2d996c.png)

## Resumen de la convocatoria

- **Cuándo:** 20 feb 2026 08:00 a 20 mar 2026 14:00 (Europe/Madrid / UTC+1)
- **Nombre de contacto:** Francina Solé, Carme Corominas
- **Correo de contacto:** captaciotalent.sp@upc.edu
- **Teléfono de contacto:** 934010880
- **Calendario (iCal, local):** [`fpu-upc-2026/fpu-upc-2026.ics`](./fpu-upc-2026/fpu-upc-2026.ics)

## Aspectos destacados

### Perfil objetivo

- Personal investigador predoctoral (etapa R1)

### Retribución

- 1.º y 2.º año: 20.063,96 € brutos/año
- 3.º y 4.º año: 25.079,88 € brutos/año
- La actividad docente se retribuirá de acuerdo con el acuerdo del Consejo de Gobierno de la UPC

### Solicitudes

- Del 20/02/2026 al 20/03/2026 a las 14:00 h

## Texto completo de la convocatoria (traducción al español)

### Objetivo

Se convocan un total de 99 ayudas FPU-UPC para promover la formación de personal investigador mediante la concesión de ayudas para la contratación de personal investigador predoctoral en el año 2026, permitiendo la dedicación a tiempo completo a estudios conducentes a la obtención del título de doctor.

Como novedad de esta convocatoria, se realizarán dos ediciones: una al final del segundo semestre del curso 2025–26 y otra al inicio del curso 2026–27.

### Requisitos generales

Para ser admitidas en el proceso de selección, las personas solicitantes deben cumplir los siguientes requisitos en el momento de aceptación de la ayuda:

- Estar admitidas en un programa de doctorado con compromiso de matricularse de tutela de tesis en el curso 2026–27, o disponer de admisión condicional a un programa de doctorado. Para la primera edición, las personas solicitantes deberán comprometerse a formalizar la matrícula cuando se abra el período de inscripción (aprox. septiembre/octubre de 2026). En caso contrario, se revocará la ayuda.
- También podrán participar si ya están matriculadas en tutela académica de un programa de doctorado en el curso anterior en el momento de aceptar la ayuda. Los programas de doctorado deben ser de la UPC.
- No haber disfrutado de otra ayuda de naturaleza similar (contrato predoctoral) por una duración superior a 12 meses al final del plazo de solicitud.
- No estar en posesión del título de doctor/a.
- No haber disfrutado, total o parcialmente, de esta ayuda en convocatorias anteriores.
- Disponer del aval de una persona PDI de la UPC que actúe como director/a de tesis, con experiencia investigadora acreditada y vinculación permanente a la UPC, o contrato laboral que supere la duración de cuatro años de la ayuda. En caso de codirección, al menos una de las personas directoras debe cumplir este requisito. Un/a director/a de tesis no podrá dirigir o codirigir más de una FPU, ni dirigir a una persona beneficiaria de convocatorias FPU-UPC anteriores mientras la ayuda esté vigente, salvo casos debidamente justificados valorados por el Vicerrectorado de Política de Personal Docente e Investigador.
- Acreditar conocimiento suficiente de catalán a nivel C1 del MCER. Las personas solicitantes que no dispongan de esta acreditación deberán solicitar una exención temporal conforme al Acuerdo CG/2023/03/09 del Consejo de Gobierno de la UPC.

### Duración de la ayuda

- El contrato predoctoral tendrá una duración mínima de un año y podrá renovarse anualmente, en el marco de este programa, hasta un máximo de cuatro años.

### Características

- Se convocan un total de 99 ayudas FPI-UPC 2026 que se distribuirán entre departamentos, institutos y cátedras UNESCO, conforme al documento disponible en el apartado “Documentación y soporte”.
- El personal investigador en formación deberá colaborar en tareas docentes hasta un máximo de 180 horas durante la vigencia del contrato predoctoral, sin superar 60 horas por año. El Acuerdo CG/2024/05/12, de 25 de octubre de 2024, regula la retribución de la actividad docente realizada en contratos predoctorales de formación.

### Importe de la ayuda

- 20.063,96 € brutos anuales en el primer y segundo año, y 25.079,88 € brutos anuales en el tercer y cuarto año.
- Además, todas las personas beneficiarias del programa FPI-UPC 2025 tienen derecho a matrícula gratuita de tutela de doctorado, con un máximo de cuatro matrículas gratuitas durante todo el período de la ayuda. Las tasas de gestión del expediente académico no son subvencionables.

### Presentación de solicitudes

A través de la sede electrónica de la UPC ([enlace original](https://seuelectronica.upc.edu/ca/tots-els-tramits/convocatoria-ajuts-formacio-professorat-universitari-FPU)), cumplimentando la instancia “Convocatoria de ayudas de formación de profesorado universitario (FPU-UPC 2026) para la realización de la tesis doctoral en la UPC”.

- Las personas candidatas solo podrán presentar una única solicitud.
- Cada director/a de tesis solo podrá constar como tal en una sola solicitud y deberá firmar un documento de aval, que la persona candidata adjuntará al formulario: [`fpu-upc-2026/aval-director-de-tesi-fpu-upc-2026.docx`](./fpu-upc-2026/aval-director-de-tesi-fpu-upc-2026.docx).
- En la solicitud deberá indicarse el departamento, instituto o cátedra UNESCO por el que se presenta, escogido entre las 41 opciones disponibles.

Para poder disfrutar de la ayuda, las personas seleccionadas deberán estar en posesión de las titulaciones que dan acceso al doctorado en el momento de formalizar el contrato.

Documentos de entrega **obligatoria** junto a la solicitud, siempre en formato PDF:

- Currículum vitae de la persona candidata y currículum vitae del/de la director/a de tesis, si procede.
- Copia de la carta de admisión al programa de doctorado o, en su defecto, carta de admisión provisional.
- Documento de aval del/de la director/a de tesis de la persona solicitante (usar: [`fpu-upc-2026/aval-director-de-tesi-fpu-upc-2026.docx`](./fpu-upc-2026/aval-director-de-tesi-fpu-upc-2026.docx)). Cada supervisor/a solo puede avalar una solicitud.
- Copia de certificados académicos de titulaciones no cursadas en la UPC, si procede. Debe constar la fecha de obtención del título y la nota media. En estudios realizados fuera del sistema universitario español, si el expediente no incluye la descripción del sistema de evaluación de la universidad, debe adjuntarse documentación acreditativa indicando nota máxima y mínima, y nota mínima de aprobado.
- Copia del documento de equivalencia de nota media para estudios realizados en centros extranjeros, expedido en el portal del Ministerio: https://universidades.sede.gob.es/pagina/index/directorio/Equivalencia_notas_medias/language/ca_ES.
- **Importante:** Para que departamentos/institutos/cátedras UNESCO puedan evaluar las solicitudes según sus criterios de priorización relacionados con el Anexo B.2), las personas solicitantes deben completar el documento anexo/plan de trabajo y subirlo en PDF en el apartado correspondiente de la sede electrónica: [`fpu-upc-2026/document-annex_ajuts-fpu-upc_2026.docx`](./fpu-upc-2026/document-annex_ajuts-fpu-upc_2026.docx).
- Copia del certificado de catalán C1 MCER. En caso de no disponer de él, se puede solicitar exención temporal mediante el formulario normalizado de solicitud.

Documento adicional a aportar:

- Para el cálculo de la nota media ponderada, la persona solicitante deberá aportar un documento oficial que indique la nota media de titulados de su promoción. Este documento no es necesario para títulos expedidos por la UPC y debe adjuntarse en “otros anexos”. Si no se aporta, solo se tendrá en cuenta la nota media de la persona candidata.

### Criterios de priorización de solicitudes

Los departamentos, institutos y cátedras UNESCO deben enviar los criterios que aplicarán para las ayudas que tienen asignadas.

Las respuestas deben enviarse a <captaciotalent.sp@upc.edu> antes del 30/01/2026, usando la plantilla disponible aquí: [`fpu-upc-2026/criteris-per-fpu-upc_2026.docx`](./fpu-upc-2026/criteris-per-fpu-upc_2026.docx).

- Distribución de ayudas entre departamentos/institutos/cátedras y especificación de cuáles corresponden a 1.ª y 2.ª edición: [`fpu-upc-2026/no-ajuts.pdf`](./fpu-upc-2026/no-ajuts.pdf).
- Criterios de priorización por departamento/instituto/cátedra (catalán): [`fpu-upc-2026/criteris-davaluacio.pdf`](./fpu-upc-2026/criteris-davaluacio.pdf).

### Nota específica de departamento (Matemàtiques / MAT, código 749)

Para personas directoras/candidatas que apliquen por el departamento de Matemáticas:

- En el anexo de distribución (`no-ajuts.pdf`), **MAT (749) tiene 3 ayudas**, todas en la **1.ª edición**.
- En el anexo de criterios (`criteris-davaluacio.pdf`), el apartado **“749 Matemàtiques (MAT)”** define las reglas de priorización del departamento.
- La puntuación MAT se divide en:
  - **Apartado A (expediente académico): 50%**
  - **Apartado B (evaluación de comisión ad hoc del departamento): 50%**
- Puntos relevantes específicos de MAT:
  - criterio de nota media ponderada con **mínimo 7/10**
  - se evalúa explícitamente el **CVA del/de la director/a de tesis**
  - **puntuación mínima de admisibilidad final: 7/10**

Esto confirma que existe, en la práctica, una priorización interna por departamento (“pre-call”) dentro de la convocatoria global FPU.

### Checklist MAT de solicitud (dirección + candidatura)

Para una solicitud por MAT, preparar al menos lo siguiente:

**Candidatura**
- Presentar la solicitud por la sede electrónica de la UPC (enlace oficial en la convocatoria).
- CV en PDF (obligatorio). En la plantilla del anexo/plan de trabajo, incluir el apartado de currículum de la persona candidata (**máx. 3 páginas**).
- Carta de admisión al doctorado (o admisión provisional).
- Certificados académicos (y, para estudios extranjeros, equivalencia de nota y documentación del sistema de calificación).
- Documento anexo/plan de trabajo completado y subido en PDF.
- Certificado C1 de catalán (o solicitud de exención temporal, si aplica).

**Dirección de tesis (advisor)**
- Firmar el documento oficial de aval (`aval-director-de-tesi-fpu-upc-2026.docx`) para esta candidatura.
- Asegurar que la dirección de tesis aparece en una sola solicitud.
- Aportar CV en PDF; para MAT conviene preparar una versión sólida en formato **CVA**, ya que el criterio MAT evalúa explícitamente el CVA de la dirección.

**Comprobaciones estratégicas MAT**
- Verificar que la nota media ponderada de la candidatura sea competitiva y esté en/al menos el umbral MAT (criterio con mínimo 7/10).
- Alinear el plan de trabajo con los criterios del apartado B de MAT (trayectoria, temática de tesis, plan a 4 años, adecuación y contexto de investigación del grupo).
- Objetivo de puntuación final por encima del mínimo de admisibilidad MAT (**7/10**).

### Preguntas frecuentes para departamentos / institutos / cátedras UNESCO

Listado de FAQs de la convocatoria: [`fpu-upc-2026/faqs-pels-departaments_fpu-upc-2026.pdf`](./fpu-upc-2026/faqs-pels-departaments_fpu-upc-2026.pdf)

### Procedimiento de la convocatoria

Se anunciará próximamente.

### Documentación y soporte

- Convocatoria (publicación en tablón, catalán): [`fpu-upc-2026/detall_idEns_12_idEdicte_613251.html`](./fpu-upc-2026/detall_idEns_12_idEdicte_613251.html)
- Convocatoria (PDF en español): [`fpu-upc-2026/convocatoria-fpu-upc-2026_es.pdf`](./fpu-upc-2026/convocatoria-fpu-upc-2026_es.pdf)
- Equivalencia de nota media (portal oficial): https://universidades.sede.gob.es/pagina/index/directorio/Equivalencia_notas_medias/language/ca_ES
- Calendario provisional: [`fpu-upc-2026/calendari-provisional-fpu-upc-2026.pdf`](./fpu-upc-2026/calendari-provisional-fpu-upc-2026.pdf)
- Documento de aval de dirección de tesis: [`fpu-upc-2026/aval-director-de-tesi-fpu-upc-2026.docx`](./fpu-upc-2026/aval-director-de-tesi-fpu-upc-2026.docx)
- Documento anexo / Plan de trabajo: [`fpu-upc-2026/document-annex_ajuts-fpu-upc_2026.docx`](./fpu-upc-2026/document-annex_ajuts-fpu-upc_2026.docx)
- Guía para elegir la escala de conversión de notas (catalán): [`fpu-upc-2026/instruccions_decl_equivalencies_upc.pdf`](./fpu-upc-2026/instruccions_decl_equivalencies_upc.pdf)
- Marco de retribución docente (acuerdo Consejo de Gobierno UPC): [`fpu-upc-2026/cg_2024_05_12-marc-per-a-la-realitzacio-voluntaria-i-cobrament-dhores-de-docencia-per-part-del-personal-investigador-en-formacio.pdf`](./fpu-upc-2026/cg_2024_05_12-marc-per-a-la-realitzacio-voluntaria-i-cobrament-dhores-de-docencia-per-part-del-personal-investigador-en-formacio.pdf)

Para más información, contactar con: <captaciotalent.sp@upc.edu>

En toda producción científica, y especialmente en las publicaciones que se deriven de ella, debe constar la afiliación de la persona beneficiaria a la Universitat Politècnica de Catalunya, de acuerdo con el punto 2 de la normativa reguladora de la estructura de unidades de investigación de la UPC (CG 6/11 2006) y con las directrices institucionales de afiliación de la UPC. En la sección de agradecimientos de cualquier publicación o comunicación deberá incluirse la siguiente redacción: “El/la autor/a correspondiente/primer/a/segundo/a/etc. agradece a la Universitat Politècnica de Catalunya el apoyo financiero proporcionado mediante la ayuda predoctoral FPU-UPC (número de referencia de la ayuda)”.

## Palabras clave

- Carrera investigadora
- R1
